...
import { 
  Zap, ChevronDown, Brain, Target, Star, Trophy, MessageCircle, Briefcase, Globe
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white overflow-x-hidden">
      ...
      <header className="w-full p-6 flex justify-between items-center max-w-[1200px] mx-auto z-50 sticky top-0 bg-white/80 backdrop-blur-md">
        ...
        <div className="hidden md:flex items-center gap-2 text-slate-400 font-bold uppercase tracking-wider text-sm cursor-pointer hover:text-violet-600 transition-colors">
      ...
      <main className="flex flex-col lg:flex-row items-center justify-center gap-12 w-full max-w-[1200px] mx-auto px-6 mt-8 lg:mt-20 mb-20">
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left space-y-8 max-w-[500px] order-2 lg:order-1">
          ...
          <p className="text-lg text-slate-500 font-medium leading-relaxed">
             Learn communication, leadership, and emotional intelligence with bite-sized, gamified lessons.
          </p>
          <div className="w-full flex flex-col sm:flex-row gap-3">
            <Link href="/get-started" className="w-full sm:w-auto flex-1">
              <button className="btn-primary w-full py-4 text-lg shadow-xl shadow-violet-100 hover:scale-[1.02] active:scale-95 transition-all">Get Started</button>
            </Link>
        ...
        <div className="relative w-[320px] h-[320px] lg:w-[500px] lg:h-[500px] order-1 lg:order-2">
           ...
           <div className="absolute bottom-20 left-0 bg-white p-3 rounded-2xl shadow-lg animate-bounce duration-[4000ms] hidden lg:block">
              <Trophy className="text-violet-500" size={32} fill="currentColor" />
           </div>
        </div>
      </main>
      ...
      <section className="w-full bg-slate-900 py-12 text-white border-y-8 border-violet-600">
         <div className="max-w-[1000px] mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            ...
            <div className="space-y-1"><h3 className="text-4xl font-black text-sky-400">15 mins</h3><p className="font-bold text-slate-300 uppercase tracking-widest text-sm">Daily Practice</p></div>
         </div>
      </section>
      ...
      <section className="py-20 px-6 max-w-[1200px] mx-auto w-full">
         ...
         <div className="grid md:grid-cols-3 gap-8">
            <FeatureCard icon={<Zap size={40} className="text-violet-500" fill="currentColor" />} title="Gamified Learning" desc="Earn points, keep your streak alive, and climb the leaderboards." />
            <FeatureCard icon={<Brain size={40} className="text-pink-500" />} title="Science-backed" desc="Our spaced repetition method ensures you remember what you learn." />
            <FeatureCard icon={<Target size={40} className="text-sky-500" />} title="Real-world Scenarios" desc="Practice handling difficult conversations, interviews, and presentations." />
         </div>
      </section>
      ...
      <footer className="w-full bg-slate-50 py-10 border-t-2 border-slate-100 text-center">
         ...
         <p className="text-slate-400 font-bold text-sm">© 2026 Castpotro Inc.</p>
      </footer>
    </div>
  );
}
