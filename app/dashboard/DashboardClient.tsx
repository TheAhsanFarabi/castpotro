    ...
    <div className="flex w-full max-w-[1920px] mx-auto">
              ...
              {selectedCourse.units.sort((a: any, b: any) => a.order - b.order).map((unit: any, index: number) => {
                  ...
                  return (
                    <div key={unit.id} className={`w-full flex flex-col items-center ${isUnitLocked ? "opacity-60 grayscale" : ""}`}>
                      <div className={`w-full text-white rounded-3xl p-8 mb-8 flex justify-between items-center shadow-lg relative overflow-hidden ${index % 2 === 0 ? "bg-[#0ea5e9] shadow-sky-200" : "bg-pink-500 shadow-pink-200"}`}>
                  ...
                  <div key={course.id} onClick={() => handleCourseClick(course.id)} className={`group border-2 rounded-3xl p-6 cursor-pointer transition-all hover:shadow-xl hover:-translate-y-1 relative overflow-hidden h-full flex flex-col justify-between min-h-[260px] backdrop-blur-md ${isEnrolled ? "bg-white/80 border-slate-200 hover:border-sky-300" : "bg-white/60 border-slate-200 hover:border-pink-300"}`}>
                    ...
                    <div className="w-full pt-4 border-t border-slate-200/60">
                      {isEnrolled ? (
                        <>
                          <div className="mb-4">
                            <div className="flex justify-between items-center text-xs font-bold text-slate-400 mb-2"><span className="uppercase tracking-wider">Progress</span><span className="text-[#0ea5e9]">{progressPercent}%</span></div>
                            <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-[#0ea5e9] rounded-full transition-all duration-1000 ease-out" style={{ width: `${progressPercent}%` }}></div></div>
                          </div>
                          <button className="w-full py-3.5 rounded-xl border-b-4 border-slate-200 bg-slate-100 text-slate-500 font-extrabold uppercase tracking-wider text-sm group-hover:bg-[#0ea5e9] group-hover:text-white group-hover:border-sky-700 transition-all flex items-center justify-center gap-2">{progressPercent > 0 ? "Continue" : "Start Learning"} <Play size={16} fill="currentColor" /></button>
                        </>
                      ) : (
          ...
          <div className="bg-white/90 backdrop-blur-xl w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-200 border border-white/50">
            ...
            <div className="p-8 text-center flex flex-col items-center">
              {enrollStatus === "success" ? (
                ...
                <>
                  <div className="w-24 h-24 bg-sky-100 text-[#0ea5e9] rounded-3xl flex items-center justify-center mb-6 shadow-sm transform -rotate-6"><Briefcase size={48} /></div>
      ...
      <div className="hidden xl:flex flex-col w-[240px] 2xl:w-[300px] bg-white/40 backdrop-blur-xl p-6 h-screen sticky top-0 overflow-y-auto custom-scrollbar gap-6 shrink-0 border-l-2 border-white/40">
        ...
        <Link href="/dashboard/plus">
          <div className="bg-gradient-to-br from-[#0ea5e9] to-pink-500 rounded-2xl p-6 text-white relative overflow-hidden group cursor-pointer shadow-lg shadow-sky-200 transition-transform hover:scale-[1.02] active:scale-95">
            <div className="relative z-10">
