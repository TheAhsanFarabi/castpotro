...
import { 
  ...
  Sparkles, Crown, Search, ArrowRight, Zap, Bell, Video, Loader2
} from 'lucide-react';
import { registerForEvent } from "@/app/actions";
import { useRouter } from 'next/navigation';

// Helper to map DB types to Icons & Colors
const getEventStyle = (type: string) => {
  const t = type || "General";
  if (t.includes('Lit') || t.includes('Book')) return { icon: BookOpen, color: "bg-emerald-500", bg: "bg-emerald-50", text: "text-emerald-600" };
  if (t.includes('Soc') || t.includes('Cult')) return { icon: Globe, color: "bg-indigo-500", bg: "bg-indigo-50", text: "text-indigo-600" };
  if (t.includes('Deb') || t.includes('Speak')) return { icon: MessageCircle, color: "bg-rose-500", bg: "bg-rose-50", text: "text-rose-600" };
  if (t.includes('Med') || t.includes('Tech')) return { icon: Mic, color: "bg-amber-500", bg: "bg-amber-50", text: "text-amber-600" };
  return { icon: Zap, color: "bg-purple-500", bg: "bg-purple-50", text: "text-purple-600" };
};
...
export default function EventsClient({ events }: { events: any[] }) {
  ...
  const handleRegister = async (eventId: string) => {
    ...
    await registerForEvent(eventId); // Calls server action
    setLoadingId(null);
    router.refresh(); // Refreshes UI with new registration status
  };
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-slate-200 rounded-2xl p-4 mb-6 sticky top-0 z-30 shadow-sm flex items-center justify-between">
              ...
              <div className="flex gap-3">
                 <div className="flex items-center gap-2 text-purple-600 font-bold bg-purple-50 px-4 py-2 rounded-xl border border-purple-100 shadow-sm">
                    <Ticket size={18} /> {ticketCount} <span className="hidden sm:inline">Tickets</span>
                 </div>
              </div>
          </div>
                    ...
                    return (
                        <div key={event.id} className="group bg-white border-2 border-slate-100 rounded-[24px] p-6 hover:border-slate-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-4 relative z-10">
                               <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-md ${style.color}`}>
                                   <EventIcon size={24} />
                               </div>
                               <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${style.bg} ${style.text}`}>
                                   {event.type}
                               </div>
                            </div>
                            ...
                            <div className="flex-1 relative z-10">
                               ...
                               <div className="flex items-center gap-4 text-xs font-bold text-slate-400 mb-6">
                                   <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md border border-slate-100"><Calendar size={14} /> {dateString}</span>
                                   <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md border border-slate-100"><Clock size={14} /> {timeString}</span>
                                   <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md border border-slate-100"><Users size={14} /> {event.capacity} Spots</span>
                               </div>
                            </div>
                            ...
                            {event.isRegistered ? (
                                <div className="space-y-2">
                                    ...
                                    {event.meetingLink && (
                                        <a href={event.meetingLink} target="_blank" className="w-full py-3 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 bg-[#0ea5e9] text-white shadow-md hover:bg-sky-600 relative z-10">
                                ...
                                <button 
                                    ...
                                    `}
                                >
                                    {loadingId === event.id ? <><Loader2 size={16} className="animate-spin"/> processing</> : "Join Event"}
                                </button>
                            )}
                            
                            <div className={`absolute inset-0 opacity-0 group-hover:opacity-5 transition-opacity duration-300 ${style.color}`}></div>
                        </div>
                    );
                })}
            </div>
                     ...
                     {myTickets.map(e => {
                         ...
                         return (
                             <div key={e.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border-2 border-slate-100 hover:border-purple-200 hover:shadow-md transition-all cursor-pointer">
                                 <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white shadow-sm ${style.color}`}>
                                     <EventIcon size={18} />
                                 </div>
          ...
          <div className="border-2 border-slate-200 rounded-[24px] p-5 bg-white shadow-sm">
             ...
             <div className="space-y-4">
                 <div className="flex gap-3 items-start">
                     ...
                     <div>
                         <p className="text-xs font-bold text-slate-600 leading-tight">Welcome to the Events Portal!</p>
                         <p className="text-[10px] text-slate-400 font-bold mt-1">Just now</p>
                     </div>
                 </div>
             </div>
          </div>
