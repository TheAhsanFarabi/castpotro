...
import ImpactDNA from "./ImpactDNA";
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 font-bold text-xs uppercase tracking-wider">
                {selectedSDG
                  ? `Filtered by: ${getSDG(selectedSDG)?.name}`
                  : "All Active Quests"}
              </p>
            </div>
          </div>
        ...
        <div className="p-8 max-w-6xl mx-auto space-y-10">
          ...
          <section className="animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-extrabold text-slate-700 flex items-center gap-2">
                <Globe size={20} className="text-emerald-500" /> Filter by
                Impact (SDGs)
              </h2>
              {selectedSDG && (
                <button
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 pt-2 custom-scrollbar px-1">
              {SDGs.map((sdg) => (
                <button
                  key={sdg.id}
                  onClick={() =>
                    setSelectedSDG(selectedSDG === sdg.id ? null : sdg.id)
                  }
                  className={`
          ...
          <section className="animate-in slide-in-from-bottom-8 duration-700">
            <h2 className="text-lg font-extrabold text-slate-700 mb-4 flex items-center gap-2">
              ...
              {selectedSDG
                ? `${getSDG(selectedSDG)?.name} Challenges`
              ...
              {filteredQuests.length === 0 ? (
                <div className="text-center py-12 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 animate-in fade-in">
                  <p className="text-slate-400 font-bold">
                    No active quests found for this category.
                  </p>
                  <button
                    onClick={() => setSelectedSDG(null)}
                    className="mt-2 text-[#0ea5e9] font-bold text-sm hover:underline"
                  >
                    View all quests
                  </button>
                </div>
                ...
                filteredQuests.map((quest) => {
                  ...
                  return (
                    <div
                      key={quest.id}
                      className="relative bg-white border-2 border-slate-100 rounded-3xl p-6 flex flex-col md:flex-row items-center gap-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 overflow-hidden group"
                    >
                      {/* Left Color Stripe */}
                      <div
                      ...
                      {/* Icon Box */}
                      <div
                        className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shrink-0 bg-slate-50 border-2 border-slate-100 transition-transform group-hover:scale-105"
                      ...
                      <div className="flex flex-col items-center gap-3 shrink-0 min-w-[100px]">
                        ...
                        {status === "APPROVED" ? (
                          <button
                            disabled
                            className="px-6 py-2 bg-green-100 text-green-600 font-bold rounded-xl text-sm cursor-default flex items-center gap-2"
                          >
                          ...
                          <button
                            disabled
                            className="px-6 py-2 bg-amber-100 text-amber-600 font-bold rounded-xl text-sm cursor-default flex items-center gap-2"
                          >
