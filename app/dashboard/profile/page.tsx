...
function AvatarCreator({ isOpen, onClose, onSave, currentData }: { isOpen: boolean; onClose: () => void; onSave: (data: AvatarData) => void; currentData: AvatarData }) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isOpen ? 'bg-black/60 backdrop-blur-sm opacity-100' : 'bg-black/0 backdrop-blur-none opacity-0 pointer-events-none'}`}>
      ...
      <div className={`bg-white rounded-[32px] p-6 w-full max-w-md shadow-2xl transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) transform ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-90 translate-y-10 opacity-0'}`}>
        ...
        <div className="flex justify-between items-center mb-6">
            ...
            <button 
                onClick={onClose} 
                className="p-2 bg-slate-100 rounded-full hover:bg-red-50 hover:text-red-500 transition-colors duration-200"
            >
                <X size={20} />
            </button>
        </div>

        {/* Live Preview with "Pop" Animation using key prop */}
        <div className="flex justify-center mb-8 py-6 bg-slate-50 rounded-3xl border-2 border-slate-100 border-dashed relative overflow-hidden group">
            ...
            <div 
                // Changing the key forces React to re-trigger the entrance animation
                ...
                className={`
                    ...
                    animate-in zoom-in-50 spin-in-12
                    relative z-10
                `}
            >
                {localAvatar.icon}
                ...
                <div className="flex flex-wrap gap-3">
                    {COLORS.map((c) => (
                        <button 
                           ...
                           className={`
                                w-10 h-10 rounded-full ${c} border-4 transition-all duration-300 ease-out
                                hover:scale-110 active:scale-90
                                ${localAvatar.color === c ? 'border-slate-300 scale-110 ring-2 ring-[#0ea5e9] ring-offset-2 shadow-lg' : 'border-transparent hover:shadow-md'}
                           `}
                        />
                    ))}
                </div>
                 ...
                 <div className="flex gap-3">
                    {SHAPES.map(s => (
                        <button
                           ...
                           className={`
                                h-10 flex-1 bg-white border-2 rounded-xl font-bold text-slate-600 text-xs 
                                transition-all duration-200 active:scale-95
                                ${localAvatar.shape === s.class 
                                    ? 'border-[#0ea5e9] bg-sky-50 text-[#0ea5e9] shadow-inner' 
                ...
                <div className="grid grid-cols-6 gap-2">
                   {EMOJIS.map(e => (
                       <button
                          ...
                          className={`
                            aspect-square flex items-center justify-center text-xl bg-white border-2 rounded-xl 
                            transition-all duration-200 hover:scale-110 active:scale-90
                            ${localAvatar.icon === e 
                                ? 'border-[#0ea5e9] bg-sky-50 shadow-inner scale-105' 
                                : 'border-slate-200 hover:border-[#0ea5e9]'}
                          `}
                       >
                           {e}
                       </button>
  ...
  return (
    <div className="flex w-full h-full">
      ...
      <div className="flex-1 overflow-y-auto bg-white relative scroll-smooth">
        ...
        {/* Top Sticky Header */}
        <div className="flex justify-between items-center sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 lg:px-10 py-4 mb-6 transition-all duration-300">
            ...
            <div className="bg-slate-50 border-2 border-slate-100 rounded-[32px] p-8 flex flex-col md:flex-row items-center gap-8 mb-8 relative overflow-hidden group">
               ...
               <div className="flex-1 text-center md:text-left space-y-2 w-full transition-all duration-300">
                  {isEditing ? (
                      <div className="space-y-3 max-w-md animate-in slide-in-from-left-4 duration-300">
                          ...
                          <input 
                            ...
                            className="w-full text-slate-500 font-bold bg-white border-2 border-slate-200 rounded-xl px-4 py-2 focus:border-[#0ea5e9] focus:outline-none transition-all shadow-sm focus:shadow-md" 
                          />
                      </div>
                  ) : (
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-4 pt-2">
                     <p className="text-slate-500 font-medium flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-100 shadow-sm text-sm hover:scale-105 transition-transform cursor-default">
