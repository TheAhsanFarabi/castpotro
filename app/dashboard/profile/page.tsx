...
function ProfileContent() {
  ...
  const generateCV = async () => {
    ...
    yPos += 15;
    doc.setFont("helvetica", "normal");
    const bioLines = doc.splitTextToSize(profile.bio || "No bio.", pageWidth - 40);
    ...
    if (certificates.length > 0) {
      ...
      certificates.forEach((cert) => {
        ...
        doc.text(`   ID: ${cert.credentialId} | ${cert.issueDate}`, 25, yPos + 5);
        doc.setFontSize(12);
        doc.setTextColor(51, 65, 85);
        yPos += 15;
      });
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-[#0ea5e9]" size={40} /></div>;
  ...
  return (
    <div className="flex w-full h-full">
        ...
        <div className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-100">
          <div className="max-w-7xl mx-auto px-6 lg:px-10 py-4 flex justify-between items-center">
            ...
            <div className="flex gap-4">
              <button onClick={() => setIsCustomizerOpen(true)} className="flex items-center gap-2 font-bold px-4 py-2 rounded-xl bg-white text-slate-600 border-2 border-slate-200 hover:border-purple-200 hover:text-purple-600 hover:bg-purple-50 transition-all duration-300 active:scale-95 shadow-sm">
                <Palette size={18} /><span className="hidden sm:block">Appearance</span>
              </button>
              <button onClick={generateCV} className="flex items-center gap-2 font-bold px-4 py-2 rounded-xl bg-slate-800 text-white hover:bg-slate-700 transition-all active:scale-95 shadow-md shadow-slate-200">
                <FileDown size={18} /><span className="hidden sm:block">Download CV</span>
              </button>
              <button onClick={() => setIsEditing(!isEditing)} className={`flex items-center gap-2 font-bold px-4 py-2 rounded-xl transition-all duration-300 border-2 active:scale-95 ${isEditing ? "bg-white text-slate-500 border-slate-200 hover:bg-slate-50" : "bg-sky-50 text-[#0ea5e9] border-sky-100 hover:bg-sky-100"}`}>
                {isEditing ? <><X size={18} /> Cancel</> : <><Edit2 size={18} /> Edit Profile</>}
              </button>
              {isEditing && (
                <div className="animate-in fade-in zoom-in duration-300">
                  <button onClick={handleProfileSave} className="flex w-full items-center justify-center gap-2 font-bold px-4 py-2 rounded-xl transition-all bg-[#0ea5e9] text-white shadow-lg shadow-sky-200 hover:bg-sky-600 active:scale-95 whitespace-nowrap">
                    <Save size={18} /> Save
                  </button>
                </div>
              )}
        ...
        <div className="px-6 lg:px-10 pb-20 max-w-7xl mx-auto pt-6">
              ...
              <div className="flex-1 text-center md:text-left space-y-2 w-full pt-6 md:pt-0 mb-2 relative z-20">
                {isEditing ? (
                  <div className="space-y-3 max-w-md animate-in slide-in-from-left-4 duration-300 mx-auto md:mx-0">
                    ...
                    <div className="text-sm text-slate-400 font-bold">{profile.handle}</div>
                  </div>
                ) : (
                ...
                <div className="flex items-center justify-center md:justify-start gap-4 pt-2">
                  ...
                  <p className="text-slate-500 font-medium flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 text-sm hover:scale-105 transition-transform cursor-default">
                    <MapPin size={14} className="text-rose-500" /> {profile.location || "Add Location"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div className="border-2 border-slate-100 rounded-2xl p-5 flex flex-col items-center gap-2 bg-white shadow-sm hover:border-orange-200 hover:-translate-y-1 hover:shadow-orange-100/50 hover:shadow-lg transition-all duration-300 group cursor-default">
              ...
              <div className="text-center"><div className="text-2xl font-black text-slate-700">{profile.coins || 0}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Coins</div></div>
            </div>
            <div className="border-2 border-slate-100 rounded-2xl p-5 flex flex-col items-center gap-2 bg-white shadow-sm hover:border-sky-200 hover:-translate-y-1 hover:shadow-sky-100/50 hover:shadow-lg transition-all duration-300 group cursor-default">
              ...
              <div className="text-center"><div className="text-2xl font-black text-slate-700">{profile.xp || 0}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total XP</div></div>
            </div>
            <div className="border-2 border-slate-100 rounded-2xl p-5 flex flex-col items-center gap-2 bg-white shadow-sm hover:border-yellow-200 hover:-translate-y-1 hover:shadow-yellow-100/50 hover:shadow-lg transition-all duration-300 group cursor-default">
              ...
              <div className="text-center"><div className="text-2xl font-black text-slate-700">{profile.league || "Bronze"}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">League</div></div>
            </div>
            <div className="border-2 border-slate-100 rounded-2xl p-5 flex flex-col items-center gap-2 bg-white shadow-sm hover:border-purple-200 hover:-translate-y-1 hover:shadow-purple-100/50 hover:shadow-lg transition-all duration-300 group cursor-default">
              ...
              <div className="text-center"><div className="text-2xl font-black text-slate-700">{certificates.length}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Certs</div></div>
            </div>
          </div>
                ...
                <div className="flex items-start gap-4 group">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <div className="font-bold text-slate-700">{profile.location || "Not set"}</div>
                    )}
                  </div>
                </div>
                <div className="flex items-start gap-4 group">
                  ...
                  <div className="flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <div className="font-medium text-slate-600 text-sm leading-relaxed">{profile.bio || "No bio yet."}</div>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="hidden xl:flex flex-col w-[240px] 2xl:w-[300px] bg-slate-50/50 p-8 h-screen sticky top-0 overflow-y-auto custom-scrollbar gap-8 shrink-0 border-l-2 border-slate-100">
        ...
        <div className="border-2 border-slate-200 rounded-[32px] p-6 bg-gradient-to-br from-white to-slate-50 shadow-sm relative overflow-hidden group">
  {/* Decorative Background Glow */}
  <div className="absolute -right-10 -bottom-10 w-32 h-32 bg-purple-500/10 rounded-full blur-3xl group-hover:bg-[#0ea5e9]/20 transition-colors duration-700"></div>

  <div className="relative z-10">
    <div className="flex items-center justify-between mb-6">
      <div className="flex flex-col">
        <h3 className="font-black text-slate-800 text-sm uppercase tracking-wider flex items-center gap-2">
          <Shield size={18} className="text-purple-500" /> NFT Your Profile
        </h3>
        <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-tighter">
          On-Chain Identity
        </p>
      </div>
      <div className="p-2 bg-purple-50 text-purple-600 rounded-xl animate-pulse">
        <Crown size={16} fill="currentColor" />
      </div>
    </div>

    <div className="space-y-4 mb-6">
      <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 shadow-xl relative overflow-hidden group/card">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center">
            <Zap size={16} className="text-white" fill="currentColor" />
          </div>
          <div>
            <div className="text-[10px] font-black text-purple-400 uppercase tracking-widest">Network</div>
            <div className="text-xs font-bold text-white">Sepolia Testnet</div>
          </div>
        </div>
        <p className="text-slate-400 text-[11px] leading-relaxed font-medium">
          Mint your soft-skill progress as a unique NFT. Increase your credibility with verified Impact DNA.
        </p>
        <div className="absolute -right-4 -bottom-4 opacity-10 text-white rotate-12 group-hover/card:rotate-45 transition-transform duration-700">
          <Hexagon size={60} />
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-2">
      <button className="w-full py-3 bg-[#0ea5e9] text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-sky-200 hover:bg-sky-600 hover:-translate-y-0.5 transition-all active:scale-95 flex items-center justify-center gap-2">
        <ImageIcon size={14} /> Mint Profile NFT
      </button>
      ...
      <a 
        ...
        target="_blank" 
        rel="noopener noreferrer"
        className="w-full py-3 bg-white text-slate-600 border-2 border-slate-100 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-50 hover:border-slate-200 transition-all flex items-center justify-center gap-2"
      >
        <ExternalLink size={14} /> Buy Sepolia ETH
      </a>
    </div>
  </div>
</div>
