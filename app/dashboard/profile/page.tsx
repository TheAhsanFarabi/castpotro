...
import { 
  ...
  LayoutTemplate, Image as ImageIcon, Camera, Sparkles, Loader2 
} from 'lucide-react';
import { useState, Suspense, useEffect } from 'react';
import { getFullUserProfile, updateAppearance, updateProfileDetails } from '@/app/actions/profile';
...
function ProfileContent() {
  ...
  // Generate CV (PDF) - Updated to use real data
        ...
        <div className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-100">
           <div className="max-w-7xl mx-auto px-6 lg:px-10 py-4 flex justify-between items-center">
               ...
               <div className="flex gap-4">
                   ...
                   {isEditing && (<div className="animate-in fade-in zoom-in duration-300"><button onClick={handleProfileSave} className="flex w-full items-center justify-center gap-2 font-bold px-4 py-2 rounded-xl transition-all bg-[#0ea5e9] text-white shadow-lg shadow-sky-200 hover:bg-sky-600 active:scale-95 whitespace-nowrap"><Save size={18} /> Save</button></div>)}
               </div>
           </div>
        </div>
               ...
               <div className="px-8 pb-8 flex flex-col md:flex-row items-center md:items-end gap-6 -mt-12 relative z-10">
                   ...
                   <div className="flex-1 text-center md:text-left space-y-2 w-full pt-6 md:pt-0 mb-2 relative z-20">
                      ...
                      <div className="flex items-center justify-center md:justify-start gap-4 pt-2">
                          <p className="text-slate-500 font-medium flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 text-sm hover:scale-105 transition-transform cursor-default"><Calendar size={14} className="text-[#0ea5e9]" /> Joined 2026</p>
                          <p className="text-slate-500 font-medium flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 text-sm hover:scale-105 transition-transform cursor-default"><MapPin size={14} className="text-rose-500" /> {profile.location || "Add Location"}</p>
                      </div>
                   </div>
               </div>
            </div>
            ...
            <div className="mb-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {certificates.length === 0 ? (
                      <div className="col-span-2 p-8 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 font-bold">No certificates earned yet. Complete a course!</div>
                  ) : (
                         ...
                         <div key={cert.id} className="bg-white border-2 border-slate-100 rounded-2xl p-4 flex items-start gap-4 hover:border-sky-200 transition-all shadow-sm group">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex gap-2 mt-3">
                                    <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 text-slate-500 text-[10px] font-bold uppercase rounded-lg border border-slate-100 hover:bg-slate-100 hover:border-slate-200 transition"><ExternalLink size={12} /> Verify</button>
                                </div>
                            </div>
                         </div>
                      ))
