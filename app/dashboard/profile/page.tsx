...
function ProfileCustomizer({ 
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 transition-all duration-300 ${isOpen ? 'bg-black/60 backdrop-blur-sm opacity-100' : 'bg-black/0 backdrop-blur-none opacity-0 pointer-events-none'}`}>
      <div className={`bg-white rounded-[32px] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) transform ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-90 translate-y-10 opacity-0'}`}>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="flex justify-center py-8 bg-slate-50 rounded-3xl border-2 border-slate-100 border-dashed relative overflow-hidden">
                        <div className={`
                            ...
                            border-[6px] border-white shadow-2xl flex items-center justify-center text-5xl font-black text-white 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {BANNERS.map((b) => (
                            <button
                                ...
                                className={`
                                    relative h-24 rounded-2xl overflow-hidden border-4 transition-all duration-300 hover:scale-[1.02] group
                                    ${localBanner.name === b.name ? 'border-[#0ea5e9] shadow-md ring-2 ring-sky-100' : 'border-transparent shadow-sm'}
                                `}
                            >
    ...
    async function fetchUser() {
      ...
      if (userData) {
        setProfile(prev => ({
          ...
          handle: userData.name ? `@${userData.name.replace(/\s+/g, '').toLowerCase()}` : prev.handle
        }));
      }
    }
    fetchUser();
  ...
  return (
    <div className="flex w-full h-full">
      ...
      <div className="flex-1 overflow-y-auto bg-white relative scroll-smooth">
        ...
        {/* Sticky Header with NEW APPEARANCE BUTTON */}
                ...
                <div className="flex gap-4">
                    ...
                    {isEditing && (
                        <div className="animate-in fade-in zoom-in duration-300">
                            <button onClick={() => setIsEditing(false)} className="flex w-full items-center justify-center gap-2 font-bold px-4 py-2 rounded-xl transition-all bg-[#0ea5e9] text-white shadow-lg shadow-sky-200 hover:bg-sky-600 active:scale-95 whitespace-nowrap">
                                <Save size={18} /> Save
                            </button>
                        </div>
                    )}
        ...
        <div className="px-6 lg:px-10 pb-20 max-w-7xl mx-auto pt-6">
            ...
            <div className="bg-white border-2 border-slate-100 rounded-[32px] overflow-hidden mb-8 relative group shadow-sm hover:shadow-md transition-all duration-500">
               ...
               <div className="px-8 pb-8 flex flex-col md:flex-row items-center md:items-end gap-6 -mt-12 relative z-10">
                   ...
                   {/* Avatar (Cleaned - Removed Overlay Icon) */}
