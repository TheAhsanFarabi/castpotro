...
//                 <td className="p-4">
//                   <span
//                     className={`text-xs font-black uppercase tracking-wider px-2 py-1 rounded-lg ${
//                       app.status === "PENDING"
...
//                         </button>
//                       </form>
//                       <form action={rejectApplicant.bind(null, app.id, job.id)}>
...
export default async function JobApplicantsPage({
  ...
  // We deeply fetch the User's Castpotro DNA alongside their application
  const job = await prisma.job.findUnique({
    where: { id: jobId },
    include: {
      applications: {
        include: {
          user: {
            include: {
              enrollments: {
                include: {
                  course: true, // Gets the course name
                  completedLessons: true, // Calculates how far along they are
                },
              },
            },
          },
  ...
  return (
    <div>
      <Link
        href="/admin/jobs"
        className="flex items-center gap-2 text-slate-400 font-bold mb-6 hover:text-slate-600 transition-colors w-fit"
      >
        <ArrowLeft size={18} /> Back to Jobs
      </Link>
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-800">{job.role}</h1>
          <p className="text-slate-500 font-medium mt-1">
            Applicants for {job.company}
          </p>
        </div>
        <div className="bg-sky-50 border border-sky-100 text-[#0ea5e9] px-5 py-2.5 rounded-xl font-black shadow-sm">
          {job.applications.length} Candidates
        </div>
      </div>

      {/* Render our new interactive table and modal */}
      <JobApplicantsClient job={job} />
    </div>
  );
}
