...
export default async function CoursesPage() {
  ...
  // Filter: Instructors only see their own courses; Admins see all.
  const courses = await prisma.course.findMany({
    where: user?.role === "INSTRUCTOR" ? { creatorId: userId } : {},
          ...
          >
            ...
            <div className="flex gap-2 mt-6 pt-4 border-t border-slate-100">
              <Link
                ...
                className="flex-1 bg-slate-800 text-white py-2 rounded-lg text-center text-sm font-bold hover:bg-slate-700"
              >
                Manage Units
              </Link>
              <form action={deleteCourse.bind(null, course.id)}>
                <button className="bg-red-50 text-red-500 p-2 rounded-lg hover:bg-red-100">
                  <Trash2 size={20} />
                </button>
              </form>
            </div>
